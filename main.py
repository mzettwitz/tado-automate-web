...
def geo_away():
    ...
    try:
        client.set_away()
        log(f"Set home mode to Away")
        return {"status": "ok", "action": "geo_away"}
    except Exception as e:
        log(f"Error setting home mode to Away: {e}")
        raise HTTPException(status_code=500, detail="Cannot set home mode to Away: {e}")

def geo_home():
    client = get_tado()
    try:
        client.set_home()
        log(f"Set home mode to Home")
        return {"status": "ok", "action": "geo_home"}
    except Exception as e:
        log(f"Error setting home mode to Home: {e}")
        raise HTTPException(status_code=500, detail="Cannot set home mode to Home: {e}")
...
@app.put("/geo/away")
def api_geo_away(request: Request):
    """Set home mode to Away"""
    check_auth(request)
    return geo_away()

@app.put("/geo/home")
def api_geo_home(request: Request):
    """Set home mode to Home"""
    check_auth(request)
    return geo_home()
